    ...
    var APP = {
        ...
        showUISlider: function() {
            ...
            $slider.each(function() {
                var $this = $(this);
                var format = new Intl.NumberFormat('th', {
                    style: 'currency',
                    currency: 'THB',
                ...
                options.slide = function(event, ui) {
                    if (options.type === 'currency') {
                        $this.parents('.slider-range').find(".amount").val(format.format(ui.values[0]) + " ถึง " + format.format(ui.values[1]));
                    }
                    if (options.type === 'sqrwa') {
                        $this.parents('.slider-range').find(".amount").val(ui.values[0] + "  ตร.วา ถึง " + ui.values[1] + " ตร.วา ");
                    }
                };
                $this.slider(options);
                if (options.type === 'currency') {
                ...
                if (options.type === 'sqrwa') {
                    $this.parents('.slider-range').find(".amount").val($this.slider("values", 0) + " ตร.วา ถึง " + $this.slider("values", 1) + " ตร.วา ");
                }
