...
$path = "../image/m_img/"; //สร้างไฟล์สำหรับเก็บไฟล์ใหม่
    ...
    if (isset($_POST['submit'])) {
        while ($row = mysqli_fetch_array($result)) {
            if ($_POST['email'] == $row['email']) {
                ...
                echo "</script>";
                header('Refresh:0; url=../page/addstaff.php');
                $x = 1;
            }
        ...
        if ($x !== 1) {
            $sql = "INSERT INTO users(email,password,name,tel,address,utype,img,birth_date,id_card) VALUES('$email','$password','$name','$tel','$address','staff','$newname','$birth_date','$id_card') ";

            $result2 = mysqli_query($con, $sql)or die(mysqli_error($con));

            echo '<script> window.location.href = "../page/staffs.php";alert("เพิ่มข้อมูลสำเร็จ")</script>';
